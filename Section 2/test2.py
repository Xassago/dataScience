...
for userId in data.keys():
    ...
    for case in cases:
        tmp_score = {"program_rate": getPro(case), "debug_rate": getDebug(case), "early_success_degree": getEarly(case), "finish_degree": getFinish(case)}
...
for value in score_data.keys():
    ...
    var = math.sqrt(sum(map(lambda x:(x-avg)*(x-avg),tmp_list))/(len(tmp_list)-1)) #因为取指标时需要满足严苛的筛选，比总数少很多，故认为取出的数据为样本
...
for userId in data.keys():
    ...
    direc = {"str": sorted(string_score), "line": sorted(line_score), "arr": sorted(arr_score), "find": sorted(find_score),
...
with open("furtherScore.json", "w", encoding='utf-8') as f:
