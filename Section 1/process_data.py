...
for caseid in data.keys():
    ...
    for case in cases:
        ...
        if len(records)>= 4:
            ...
            data_to_process[caseid][case["user_id"]]["program_rate"] = math.log(to_cal,math.e) if to_cal>0 else INVALID
        else:
            data_to_process[caseid][case["user_id"]]["program_rate"] = INVALID


# debug_rate 只考虑大于0的数据，单位：h^(-1)
# 理由：攻克用例速度越快越好
...
for caseid in data.keys():
    ...
    for case in cases:
        ...
        if len(records)>= 3:
            ...
            data_to_process[caseid][case["user_id"]]["debug_rate"] = math.log(math.sqrt(debug_rate/up_times),math.e) if up_times>0 else INVALID
        else:
            data_to_process[caseid][case["user_id"]]["debug_rate"] = INVALID


# early_success_degree 只考虑大于0的数据，单位：1
# 理由：分数提高越早越好
for caseid in data.keys():
    cases = data[caseid]
    for case in cases:
        records = case["upload_records"]
        if len(records)>= 4:
            # 此为折线下面积所占比例的计算方法 early_success_degree avg for all caseids: 0.37581113584213893
            ...
            #     span += up_span
            # to_cal = early_success_degree/span if span else last_success_degree
            # data_to_process[caseid][case["user_id"]]["early_success_degree"] = math.log(to_cal,math.e) if to_cal>0 else INVALID

            # 此为折线值求平均的计算方法 early_success_degree avg for all caseids: 0.5001851243852434
            ...
            data_to_process[caseid][case["user_id"]]["early_success_degree"] = to_cal if to_cal>0 else INVALID
        else:
            data_to_process[caseid][case["user_id"]]["early_success_degree"] = INVALID


# finish_degree 单位：h^(-1)
# 理由：题目完成度越高越好
show_once = True
for caseid in data.keys():
    cases = data[caseid]
    for case in cases:
        ...
        if len(records)>= 4:
            ...
            data_to_process[caseid][case["user_id"]]["finish_degree"] = math.log(to_cal, math.e) if to_cal > 0 else INVALID
...
with open('intermediate_case_data.json', 'w',encoding='utf-8') as w:
