...
f = open('intermediate_case_data.json', encoding='utf-8')
res = f.read()
data = json.loads(res)
...
for caseId in caseIds:
    if len(data[choose][caseId]) >= 0:  # 按有效数据个数筛选
        ...
        num_all += 1
print("各题有效数据个数:",count)
print("有效数据多于一定数目的可用题目",canBeUsed)
...
alpha = 0.004  # 取α为0.1
refuse_bound = z_alpha[alpha / 4]  # 拒绝与临界值为Zα/4
print(refuse_bound)
n = len(canBeUsed)
...
print("各题u1, u2：")
num_valid = 0
for key in u1u2_ofEachCase.keys():  # 输出各题偏度峰度观察值
    print(key, u1u2_ofEachCase[key])
    num_valid += 1
print("有效占比:",num_valid,"/",num_all,"=",num_valid/num_all)
